...
server <- function(input, output) {
  ...
  observeEvent(
    ...
    handlerExpr = {
      ...
      DFS_title_1 <- ifelse(input$Extent_of_resection == 1, DFS_title, "")
      
      tumor_size_dfs_1 <- ifelse(input$Extent_of_resection == 1, paste("Points based on tumor size: ", tumor_size_dfs), "")
      
      fnclcc_dfs_1 <- ifelse(input$Extent_of_resection == 1, paste("Points based on FNCLCC Grade: ", fnclcc_dfs), "")
      
      Histologic_dfs_1 <- ifelse(input$Extent_of_resection == 1, paste("Points based on Histologic subtype: ", Histologic_dfs), "")
      
      Multifocality_dfs_1 <- ifelse(input$Extent_of_resection == 1, paste("Points based on Multifocality: ", Multifocality_dfs), "")
      
      DFS_1 <- ifelse(input$Extent_of_resection == 1, paste("Total points: ", total_point_dfs, "| 7 years DFS: ",round( DFS_7_years, digits = 3)), "")
